...
//      pg_tex_handle[0] = pg.getTexture().glName;
  ...
  public void setup() {
    ...
    // processing font
    font = createFont("x.ttf", Integer.parseInt(lines[0]));
...
//    drawText(pg_obstacles); // use text as obstacle
  ...
  public void drawText(PGraphics pg){
    ...
    float px, py, line_height = Integer.parseInt(lines[0])*1.2;
    px = viewport_w/2;
    py = 100+line_height;
...
//    pg.text("Simulation", px, py);
    if(Integer.parseInt(lines[1])==0){pg.textAlign(LEFT);px=50;}
    else if(Integer.parseInt(lines[1]) == 1)pg.textAlign(CENTER);
    else if(Integer.parseInt(lines[1]) == 2){pg.textAlign(RIGHT);px=viewport_w-50;}
    else pg.textAlign(CENTER);
    pg.fill(Integer.parseInt(lines1[0]),Integer.parseInt(lines1[1]),Integer.parseInt(lines1[2]));
    for(int i = 2;i<lines.length;i++){
      pg.text(lines[i],px,py-50);
      py += line_height;
    }
...
//    image(pg_text     , 0, 0);
  ...
  public void keyReleased(){
    ...
    if(key == 's') saveFrame("painting-######.png");
  }
